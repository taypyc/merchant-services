...
$(function() {
  ...
  $.validator.addMethod('validPhone', function (value, element) {
    return this.optional(element) || /^\+1\(\d{3}\)\s\d{3}-\d{4}$/.test(value);
  }, function(params, element) {
    return 'Phone format +1(XXX) XXX-XXXX'
  });
  ...
  // validator helpers
  function validatorErrorPlacement(error, element) {
    var elem = $(element);
  ...
  function validatorHighlight(element, errorClass, validClass) {
    var elem = $(element);
    if (elem.hasClass("select-item")) {
  ...
  function validatorUnighlight(element, errorClass, validClass) {
    var elem = $(element);
    if (elem.hasClass("select-item")) {
  ...
  $('.partners-form').each(function() {
    $(this).validate({
      ...
      highlight: function (element, errorClass, validClass) {
        validatorHighlight(element, errorClass, validClass);
      },
      unhighlight: function (element, errorClass, validClass) {
        validatorUnighlight(element, errorClass, validClass);
      }
    });
  });
      ...
      submitHandler: function(form) {
        ...
        $.ajax({
          ...
          complete: function(data) {
            if (typeof data.responseJSON === 'object') {
              if (data.responseJSON.response == 'success') {
                var dataReturnJson = stepsControl.parseJson(data.responseJSON.d);
                if(typeof dataReturnJson === 'object' && "redirect_link" in dataReturnJson) {
                  window.location.href = dataReturnJson.redirect_link;
                  return;
                } else if(data.responseJSON.r.length > 0) {
                  window.location.href = data.responseJSON.r;
                  return;
                } else {
                  alert('Undefined error, please try again later.');
                ...
                var filesError = 'Please select JPG, PNG or PDF File, <span style="white-space: nowrap">maximum size of image: 10 MB.</span>';
  ...
  $('[data-mask-an]').each(function() {
    $(this).rules('add', {minlength: 5, maxlength: 20});
  });
