...
gulp.task('browser-sync', () => {
  ...
  browserSync.init({
    proxy: '127.0.0.1/' + projectFolder + siteFolder + '/www/',
    notify: false,
...
gulp.task('css:build', gulp.series('css:dev', () => {
  ...
  return gulp.src(['!www/css/vendors.css', '!www/css/theme.css', 'www/css/*.css'])
    .pipe($.sourcemaps.init())
    .pipe($.postcss([
      autoprefixer({
        cascade: false
...
gulp.task('js:deploy', () => {
  return gulp.src('www/js/*.js')
    .pipe($.if(['main.js','theme.js'], $.uglify()))
...
gulp.task('files:deploy', () => {
  return gulp.src(['www/{*.*,.*}', '!www/svg.html', 'www/vendor/**/*', 'www/php/**/*', 'www/css/fonts/**/*', 'www/img/favicon/**/*'], {base: 'www'})
