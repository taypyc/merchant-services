...
class ViewControl {
  ...
  public function __construct() {
    $site_root_env = getenv('SITE_ROOT');
    $this->site_root = $site_root_env;
    ...
    $dapp_root_env = getenv('DAPP_ROOT');
    if($dapp_root_env !== false) {
      $dapp_root = $_SERVER['DOCUMENT_ROOT'] . getenv('DAPP_ROOT');
    } else {
      $dapp_root = $_SERVER['DOCUMENT_ROOT'] . $site_root_env . 'php/scripts';
    }
    require_once $dapp_root . '/MarkupBuilder.php';
    $this->markup_builder = new MarkupBuilder(['dapp_root' => $dapp_root, 'flow' => getenv('DAPP_FLOW'), 'markup' => getenv('DAPP_MARKUP'), 'transfer_protocol' => $this->transfer_protocol]);
  }
  ...
  public function display_head($head_data) {
    ...
    $meta_og_img = (array_key_exists('meta_og', $head_data) && array_key_exists('img', $head_data['meta_og'])) ? $head_data['meta_og']['img'] : $this->transfer_protocol . "://{$_SERVER['HTTP_HOST']}" . "/img/main/logo.svg";
    $meta_og = <<<EOD
...
EOD;
  ...
  public function page_end() {
    ...
    $footer_assets .= $this->markup_builder->include_footer_assets();
