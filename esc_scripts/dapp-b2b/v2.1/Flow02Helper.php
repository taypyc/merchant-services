...
class Flow02Helper {
  ...
  private $available_steps = ['0', '1', '2', '3', '4', '5'];
  ...
  public function check_required_data($data, $file_info) {
    ...
    switch ($data['step']) {
      ...
      case '3':
        if(
          $this->check_data_key_length($data, ['entity_type','state_incorporation','property_ownership','tax_id','business_started_date','actual_cash_advance_company','bank_credit'])
        ) {
          $result = true;
        }
        break;
      ...
      case '4':
        if(
          $this->check_data_key_length($data, ['step_ts'])
        ) {
          $result = true;
        }
        break;
