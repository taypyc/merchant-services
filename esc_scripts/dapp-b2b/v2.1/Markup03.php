...
class Markup {
  ...
  public function build_header($settings = []) {
    if(array_key_exists('header_type', $settings) && $settings['header_type'] == 'no_link') {
      return $this->get_markup_from_file('markup/markup_01_header_without_link.php', $settings);
    }
  ...
  public function build_footer($settings = []) {
    $available_types = [
      'lp-footer' => 'markup/markup_03_lp_footer.php'
    ];
    if(array_key_exists('type', $settings) && in_array($settings['type'], array_keys($available_types))) {
      return $this->get_markup_from_file($available_types[$settings['type']]).$this->build_page_end_dom();
    }

    return $this->get_markup_from_file('markup/markup_04_footer.php');
  }
  ...
  public function get_form_signup($continue_signup_data) {
    ...
    return $this->get_markup_from_file('markup/markup_03_quickstart.php', $continue_signup_data);
  }
  ...
  public function get_thx_content() {
    return $this->get_markup_from_file('markup/markup_04_thx_content.php');
  }

  public function get_states_select_options() {
    return $this->get_markup_from_file('markup/provinces_options.php');
  }

  public function build_page($settings) {
    $page_to_content = [
      'privacy-policy' => 'markup_04_privacy_policy',
      'lp-index-page' => 'markup_03_lp_index_page',
    ];
    $page = $this->page;
